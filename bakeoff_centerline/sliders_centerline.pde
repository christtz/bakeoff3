...
void setup() {
  ...
  size(512, 896); //set this, based on your sceen's PPI to be a 2x3.5" area.
...
void draw() {
  ...
  text("Trial " + (trialIndex+1) + " of " +trialCount, width/2, inchesToPixels(.25f));
    ...
    //==========DRAW LINE TO CONNECT SQUARES===============
pushMatrix();
translate(width/2, height/2);
//translate(width/2, height*3/4);

float centerX = t.x + screenTransX;
float centerY = t.y + screenTransY;

strokeWeight(3);
stroke(0);

line(centerX, centerY, 0,height*1/4); 

noStroke();
fill(255,255,255, 200);

ellipse(0,height*1/4, (screenZ)/2, (screenZ)/2);
ellipse(centerX, centerY, (t.z)/2, (t.z)/2);
...
//ellipse(0,0, (screenZ)/2, (screenZ)/2);
